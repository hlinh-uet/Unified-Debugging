...
from .evidence import register_program_evidence
...
def _cache_cpg_result(key: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _register_cpg_evidence(key, stored)
    while len(_CPG_TOOL_CACHE) > _CPG_TOOL_CACHE_MAX:
        _CPG_TOOL_CACHE.popitem(last=False)
    return copy.deepcopy(stored)


def _register_cpg_evidence(
    key: str,
    stored: Dict[str, Any],
) -> None:
    """Index the existing cache value without changing provider output."""
    engine = stored.get("engine") or {}
    try:
    ...
    try:
        register_program_evidence(
            namespace="static_program_analysis",
            identity=key,
            payload=stored,
            producer=str(
                engine.get("provider")
            ...
            symbol=str(request.get("function_name") or ""),
            parameters={
                "tool": request.get("tool"),
                "provider_mode": request.get("provider_mode"),
            },
            metadata={
                "result_count": len(stored.get("results") or []),
            },
            # ``stored`` is already the protected provider-cache snapshot.
            # Sharing its reference avoids duplicating every CPG result.
            retain_payload_reference=True,
        )
    except Exception:
        # The registry is observational and must never change provider logic.
        return
