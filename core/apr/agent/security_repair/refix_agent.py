...
def refix_policy_rules() -> List[str]:
    return [
        "Security ReFix: refine the guard, size/index/allocation calculation, lifetime transition, or cleanup/error path for the evidence-matched unsafe_paths[] sink.",
        "If the previous patch chose an unsupported unsafe path, switch to the path supported by validation feedback and failure evidence.",
        "Any new or refined guard must dominate the chosen sink and execute before pointer/index/buffer use or pointer/offset advancement.",
        "Any new fail-closed return/goto must satisfy cleanup_obligations from the chosen unsafe path.",
...
def _build_patch_diagnosis_hints(
    *,
    ...
    if string_length_failure:
        hints.append(
            "The focused failure discusses string length validation and numeric/decimal parsing. "
            "Do not classify unchecked string content as numeric before the target's existing validation contract allows it."
        )
...
def run_security_refix_agent(
    *,
    ...
    artifact = write_llm_step_artifact(
        ...
        step_name=f"security_refix_agent_round_{refix_round:02d}",
        prompt=prompt,
        response=response or "",
        status="generated" if response else "llm_failed",
        error="" if response else "security_refix_agent_no_response",
    )
